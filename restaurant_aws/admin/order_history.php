					...
					<div class="container">
						<div class="row">
							<div class="col-md-12">
								...
								<div class="col-md-3">
									<input type="text" class="name" type="text" placeholder="Enter name" id="name" >
								</div>
								<div class="col-md-3">	<a class="btn btn-info"  onClick="history_records()">Search</a></div>
							</div>
						</div>
					<div class="row" style="padding: 12px 1px">
						<div class="col-md-12">
							<div class="col-md-6"><input type="text" class="search form-control" placeholder="Filter Records" id="filter"></div>
						</div>
					</div>
											...
											<table class="table table-bordered results tablesorter" id="myTable">
												...
												<?php
													...
													for ($i = 0; $i < $numItem; $i++) 
													{
														...
														if($AllOrders[$i]['order_status'] == 'ORP' && ($_SESSION['user_type'] == 'admin' || $_SESSION['user_type'] == 'operator'))
															continue;
														...
														echo '<tr>';
															...
															else if($AllOrders[$i]['order_status'] == 'ORPOPG')
															{
															...
															else if($AllOrders[$i]['order_status'] == 'ORPF')
															{
															...
															else if($AllOrders[$i]['order_status'] == 'ORPOTP')
															{
...
function history_records()
	{
		try
		{
		 ...
		 if (!/^[a-zA-Z]*$/g.test(document.getElementById("name").value)) 
			{
				var msg1="Only" + ' A To Z' + ',' +' a TO z  characters are allowed in search box';
				alert(msg1);
			...
			xmlhttp.open("GET", "order_history_datewise.php?order_date="+order_date_from+"&order_date_to="+order_date_to+"&name="+name,true);
			xmlhttp.send();
	}
	catch (ex)
	{
