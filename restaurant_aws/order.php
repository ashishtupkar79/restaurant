<?php
	...
	$extra_charge_amount	= $_POST['remaining_total'];
	...
	$total_discount_per=$connection->get_order_settings('order_discount');
	...
	if($dayOfWeek == 'Wednesday' && strtotime(date('H:i:s')) >= strtotime($order_from_time) && strtotime(date('H:i:s')) <= strtotime($order_to_time) && $free_meal == 'n' && $total>=500)
	{
	...
	if($free_meal == 'n'  && $order_total_amount < ($connection->get_order_settings('minuimum_order_amount')))
		$blank_data=true;
	...
	if(!$blank_data)
	{
		...
		$results=$connection->get_data("customer_master","mobile_number,fname,lname,address1,address2,address3",$where,null);
		$exist = false;
		foreach($results as $usrinfo)
		{
			if(strlen($usrinfo['mobile_number'])>0 && strlen($usrinfo['fname'])>0 &&strlen($usrinfo['lname'])>0 && strlen($usrinfo['address1'])>0 && strlen($usrinfo['address2'])>0 && strlen($usrinfo['address3'])>0)
				$exist= true;
		}
		...
		if($exist == false)
		{
			...
			else
			{
				...
				if(isset($_SESSION['login_tag']) && $_SESSION['login_tag']!="" &&  isset($_SESSION['email_id']) && $_SESSION['email_id']!="" &&  isset($_SESSION['mobile_number']) && $_SESSION['mobile_number']!="" &&  isset($_SESSION['login_verified']) && $_SESSION['login_verified']=='y')
				{
					$email_id_chk = $_SESSION['email_id']  ;
					$mobile_number = $_SESSION['mobile_number'] ;
					$login_tag_chk = $_SESSION['login_tag'] ;
					
					$rows = array("fname"=>$firstname,"lname"=>$lastname,"address1"=>$delivery_address1,"address2"=>$delivery_address2,"address3"=>$delivery_address3);
					$where = " mobile_number =".trim($mobile)." and ".$login_tag_chk."_provider_email_id='".$email_id_chk."'" ;
					$connection->update_data('customer_master',$rows,$where);
				
				}
				else
				{
					$values=array($mobile,$firstname,$lastname,$delivery_address1,$delivery_address2,$delivery_address3);
					$insertCustMast = $connection->insert_data("customer_master",$values,"mobile_number,fname,lname,address1,address2,address3");
					$provider_email_id=$email;
				}
		}
		else
		{
		...
		/* Order Master*/
			...
			$order_total_amount = $order_total_amount + $delivery_charges;
			
			

			$values=array($order_id,$mobile,$delivery_day,$delivery_time,$delivery_address1,$delivery_address2,$delivery_address3,$comments,$area,$pincode,'ORP',$total,$order_total_amount,$discount_total,$connection->get_date(),$extra_charge_amount,$email,$free_meal,$free_credit_remain,$ip,$login_tag,$provider_email_id,$firstname,$lastname,$delivery_charges,$total_discount_per);

			$OrdertMast=$connection->insert_data("order_master",$values,"orderid,mobileno,delivery_day,delivery_time,delivery_address1,delivery_address2,delivery_address3,comments,area,pincode,order_status,order_amt,order_amt_payable,order_discount,order_entry_date,extra_charges,email_id,free_meal,free_credit_remain,ip,provider,provider_email_id,ofname,olname,delivery_charges,discount_percent");
