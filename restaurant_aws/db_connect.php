...
class createConnection //create a class for make connection
{
	...
	function insert_data($table,$values,$rows = null)
    {
        ...
        if($this->tableExists($table))
        {
            ...
            for($i = 0; $i < count($values); $i++)
            {
				...
				{
					if(strpos($values[$i],'NOW()') ==false && strpos($values[$i],'DATE_ADD') ==false)
						$values[$i] = "'".$values[$i]."'";
				}
	...
	public function update_data($table,$rows,$where)
    {
        ...
        if($this->tableExists($table))
        {
            ...
            for($i = 0; $i < count($rows); $i++)
           {
			...
			$query=mysql_affected_rows($conn);
			if($query > 0)
            {
                return true; 
            }
	   ...
	   function getMyOrder_details($OrderId)
		{
			$conn=$this->connectToDatabase();
			$cartContent = array();
			$sql = "select om.bring_change_for,om.order_cancel_reason,om.delivery_charges,om.discount_percent,om.order_amt,om.order_discount,om.order_amt_payable,om.extra_charges,od.ProductID,od.Qty,od.SaleRate,mnu_mast.item_name,
	  ...
	  function getOpOrder_detail($order_id)
		{
			$conn=$this->connectToDatabase();
			$cartContent = array();
			$sql = "SELECT  om.bring_change_for,om.order_cancel_reason,om.delivery_charges,om.discount_percent,om.paytm_respmsg,om.comments,om.ofname,om.olname,om.free_meal,om.free_credit_remain,om.delivery_day,om.delivery_time,om.mobileno,om.email_id,om.delivery_day,om.delivery_time,om.area,om.pincode,om.order_amt,om.order_discount
	...
	function get_order_settings($setting)
	{
		$conn=$this->connectToDatabase();
		$time_slots = array();
		$sql="select value from order_settings where setting='$setting'";
		$result = mysql_query($sql);
		while ($row =mysql_fetch_array($result)) 
			{		
			   $setting_val= $row['value'];
			}
	...
	function get_cart($add_more_items,$address,$extra_charge)
	{
		$conn=$this->connectToDatabase();
		$remaining_total="0";
		$free_credit_remain="";
	...
	if($count_rows > 0)
	{
		...
		$Total = 0;
		$subTotal = 0;
		$minimum_total=$this->get_order_settings('minuimum_order_amount'); // 200
		$total_discount_per=$this->get_order_settings('order_discount'); //10
		$free_credit_amount = 0;
		$free_meal ='n';
		if(isset($_SESSION['email_id']) && isset($_SESSION['mobile_number'])) 
		{
			if(($current_order_number % ($this->get_order_settings('offer_free_meal_order'))) ==0) //10
			{
				$minimum_total=0;
				$free_credit_amount = $this->get_order_settings('free_meal_credit_amount'); //230
				$free_meal ='y';
			}
		}
		if($free_meal == 'y')
		...
		while ($row = mysql_fetch_array($sql_query))
		{
			...
			?>
				...
				<?php
					...
					/* 29_10_2016*/
				$dayOfWeek=date('l');
				$order_from_time = date('20:00:00');
				$order_to_time = date('22:00:00');
				if($dayOfWeek == 'Wednesday' && strtotime(date('H:i:s')) >= strtotime($order_from_time) && strtotime(date('H:i:s')) <= strtotime($order_to_time) && $free_meal == 'n' && $subTotal>=500)
				{
					$total_discount_per =25;
				}
				/* 29_10_2016*/
				...
				if($order_total_amount<$minimum_total)
	...
	<table class="table table_summary">
		...
		<?php
			if($discount_total >0)
			{
				echo '<tr>';
					echo '<td>';
						echo ' Discount ('.$total_discount_per.'%) <span class="pull-right"><i class="fa fa-inr"></i> '.$discount_total.'</span>';
					echo '</td>';
				echo '</tr>';
			}
		...
		<tr>
			<td>
				<b style="color:#D50681"> Rs. <?php echo $this->get_order_settings('minuimum_order_amount'); ?> Minimum, Remaining</b> <span class="pull-right"><i class="fa fa-inr"></i> <?php echo number_format($remaining_total, 2, '.', ''); ?></span><hr class="hr2">
			</td>
		</tr>
		<?php } ?>
...
if($order_total_amount_v < ($this->get_order_settings('on_minimum_amount_delivery_charges_applicable_for_no_delivery_charge_areas')) ) //200
{


	$where = "  concat(pincode,'~',location) ='$address' ";
	$sql_exe=$this->get_data("location_master","delivery_charges",$where,null);
	
	foreach($sql_exe as $usrinfo)
	{
		$delivery_charges= $usrinfo['delivery_charges'];
	}
	
	if($delivery_charges <= 0)
	{
		$delivery_charges=$this->get_order_settings('delivery_charges_applicable_for_no_delivery_charge_areas');
		
	}
	
	if($delivery_charges > 0)
	{
		$order_total_amount_v = $order_total_amount_v + $delivery_charges;
		
		echo '<tr><td class="delivery_charges"><hr class="hr2">Delivery Charges <span class="pull-right"><i class="fa fa-inr"></i> '.$delivery_charges.'</span></td></tr>';
			
		echo '<tr><td class="total">Grand Total<span class="pull-right"><i class="fa fa-inr"></i> '.$order_total_amount_v .'</span></td></tr>';
	}

}

else if($order_total_amount_v < ($this->get_order_settings('on_minimum_amount_delivery_charges_applicable')) ) //600
{
	$where = "  concat(pincode,'~',location) ='$address' ";
	$sql_exe=$this->get_data("location_master","delivery_charges",$where,null);
...
<?php 	} 	?>
	...
	<?php
				...
				/*$sql_order_log="select order_stop_date, order_end_date from orders_stop_log where status='y'";
				$sql_order_log_query = mysql_query($sql_order_log);
				while($row_order_log = mysql_fetch_array($sql_order_log_query))*/
				$where = " status='y' ";
				$sql_order_log_query=$this->get_data("orders_stop_log","order_stop_date, order_end_date",$where,null);
				foreach($sql_order_log_query as $row_order_log)
				{
					$from = date($row_order_log['order_stop_date']);
					$to = date($row_order_log['order_end_date']);
				}

				//$from = date('2016-11-23 06:00:00');
				//$to = date('2016-11-24 06:00:00');
				...
				else
				{
					if($add_more_items == true)
					{
						echo '<input type="hidden" name="remaining_total" id="remaining_total" value="'.number_format($remaining_total, 2, '.', '').'">';
						echo '<input type="hidden" name="order_total_amount" id="order_total_amount" value="'.($order_total_amount_v - $delivery_charges ).'">';
				...
				echo '<input type="hidden" name="delivery_charges" id="delivery_charges" value="'.$delivery_charges.'">';
