...
$PDF_HEADER_STRING="+91 7378845457, 0712 6600288";
...
elseif($payment_type=='pcod')
{
	$payment_details='Payment Type : Pay with Card on Delivery<br>';
...
$html.= '<table class="table table-bordered" border="1" cellpadding="5px"><thead><tr><th  width="40px;">#</th><th width="180px;">Item Name</th><th>Quantity</th><th>Unit Price</th><th>Total Amount</th></tr></thead><tbody>';
										 ...
										 if($OpOrder_detail[0]['order_discount'] > 0)
											$html.= '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>Discount ('.$OpOrder_detail[0]['discount_percent'].'%)</td><td align="right"><i class="fa fa-inr"></i> '.'<b>'.number_format($OpOrder_detail[0]['order_discount'], 2, '.', '').'</b>'.'</td></tr>';
										  ...
										  if($OpOrder_detail[0]['extra_charges'] > 0)
											$html.= '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>'.$connection->get_order_settings('minuimum_order_amount').' Minimum, Remaining Charges</td><td align="right"><i class="fa fa-inr"></i> '.'<b>'.number_format($OpOrder_detail[0]['extra_charges'], 2, '.', '').'</b>'.'</td></tr>';
										  ...
										  if($OpOrder_detail[0]['delivery_charges']  > 0)
										 {
												$delivery_charges=$OpOrder_detail[0]['delivery_charges'];
													...
													$html.= '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>Total Amount</td><td align="right"><i class="fa fa-inr"></i> '.'<b>'.number_format(($OpOrder_detail[0]['order_amt_payable'] - $delivery_charges), 2, '.', '').'</b>'.'</td></tr>';

												$html.= '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>Delivery Charges</td><td align="right"><i class="fa fa-inr"></i> '.'<b>'.number_format($delivery_charges, 2, '.', '').'</b>'.'</td></tr>';
												  
												   $html.= '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>Grand Total</td><td align="right"><i class="fa fa-inr"></i> '.'<b>'.number_format($OpOrder_detail[0]['order_amt_payable'], 2, '.', '').'</b>'.'</td></tr>';
										 }
										 else
										 {
...
//$pdf->ColoredTable($header, $html);
	...
	$file=dirname( __FILE__).'/invoice/'.$order_id.'_invoice.pdf';
